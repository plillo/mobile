	...
	return {
		...
		controller: function($scope, $rootScope, $window, $element){
			...
			for(var k=0; k<$scope.attributeTypes.length;k++){
				...
				var value = $scope.extra[$scope.attributeTypes[k].name].value = $scope.attributeValues.get(uuid);
			    ...
			    haUser.updateUser($scope.updatingUser)
			   	.then(
		  	        function successCallback(response) {
						...
						// TODO
						// valutare la rigenerazione del token: dovrebbe avvenire lato-server
						// ed essere invita in risposta all'update...
						...
						var currentUser = {
							...
							roles:haUser.getCurrentUser().roles,
							accessToken:haUser.getCurrentUser().accessToken,
						};
