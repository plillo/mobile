			...
			login: function(identificator, password){
			   ...
			   $http(pars).then(
				  function successCallback(response) {
					  ...
					  // decode token
					  var decrypted = haJwt.decodeToken(response.data.token);
					  var attributeValues = new Map();
					  for(var k=0; k<decrypted.attributeValues.length; k++){
						  attributeValues.set(decrypted.attributeValues[k].uuid, decrypted.attributeValues[k].value);
					  }
					  var currentUser = {
						  uuid:decrypted.uuid,
						  username:decrypted.username,
