...
//ADD 'debug' service
//====================================
angular.module('hashServices').service('debug', function() {
	this.console = [];
...
//************************************
//ADD and INITIALIZE 'user' service
//====================================
...
// $get function returning the service
this.$get = function($http, $rootScope, $window, user, backend, jwtHelper){
	var path = this.path;
	...
	return {
		...
		login: function(identificator, password){
		   var pars = {
		      method:'GET',
		      url:backend.getBackend()+'/'+this.path+'login',
		      params:{
		    	 identificator:identificator,
		    	 password:password,
		   ...
		   $http(pars).then(
		      function successCallback(response) {
	    	      ...
	    	      for (var i = 0; i < decrypted.attributeTypes.length; i++) {
	    	    	  attributeTypes.push(decrypted.attributeTypes[i]);
	    	      }
		    	  var currentUser = {
		    		 uuid:decrypted.uuid,
				     username:decrypted.username,
		...
		validateIdentificator: function(identificator, deferred, ctrl){
		   var pars = {
		      method:'GET',
		      url:backend.getBackend()+'/'+this.path+'validateIdentificator',
		      params:{
		    	 value:identificator
		      }
		   };
		...
		update: function(){
		   ...
		   var pars = {
		      ...
		      method:'POST',
		      url:backend.getBackend()+'/'+this.path+user.uid+'/update',
		      params:{
...
angular.module('hashServices').provider('broker', function() {
	...
	// $get function returning the service
	this.$get = function($q, $rootScope, debug){
		return {
			url: provider.url,
			port: provider.port,
			deferred: undefined,
			...
			connect: function(token, password){
				debug.log('Connecting to broker at '+provider.url+':'+provider.port);
				...
				if(provider.client){
					...
					provider.client.connect({
	    				...
	    				onFailure:function(error) { // NO connection
							alert('Cannot connect to the broker: '+provider.url+':'+provider.port+'['+error.errorMessage+']');
	    					service.connected = false;
	    					deferred.reject('Cannot connect to the broker: '+provider.url+':'+provider.port+'['+error.errorMessage+']');
			...
			subscribe: function(topic, topicHandler){
				debug.log('Subscribing topic: '+topic);
			...
			unsubscribe: function(topic, topicHandler){
				debug.log('Unsubscribing topic: '+topic);
			...
			send: function(status, topic, retained){
				debug.log('Sending message {status:'+status+'} to topic: '+topic);
